...
/*
    // printing the mesh and landmark Lists
    std::cout << "Mesh List: " <<endl;
	for (std::vector<std::string>::const_iterator i = meshList.begin(); i != meshList.end(); ++i)
		std::cout << *i <<endl;

	std::cout << "Landmark List: " <<endl;
	for (std::vector<std::string>::const_iterator i = landmarkList.begin(); i != landmarkList.end(); ++i)
		std::cout << *i <<endl;
*/
	std::map<std::string, std::vector<int> > landmarksMap; 
	convertLandmarksToID(landmarksMap, meshList, landmarkList);
/*
	std::cout << "landmarks Map List: " <<endl;
	for (std::map<std::string, std::vector<int> >::iterator i = landmarksMap.begin(); i != landmarksMap.end(); ++i)
...
{ 
    ...
    //std::cout << "Number of subjects is: "  << nSubj <<endl;
    ...
    if (nSubj > 0)
    {
        for (int i = 0; i < nSubj; i++)
        {
            ...
            if ( filename.substr(filename.length() - suffixe_surfSPHARM.length()) == suffixe_surfSPHARM )   
            {
            	...
            	std::cout << "suffixe : " << suffixe_surfSPHARM  << endl;
            }
            else if ( filename.substr(filename.length() - suffixe_procalign.length()) == suffixe_procalign )
            {
                suffixe_size = suffixe_procalign.length();
                std::cout << "suffixe : " << suffixe_procalign  << endl;
            }
            else
            {
            	cerr<<"suffixe not supported! : only _pp_surf_SPHARM_procalign.vtk or  _pp_surf_SPHARM.vtk"<< endl;
            	exit(0);
            }
...
void getIDlandmarks(std::string mesh, std::string landmarks, std::vector<int> &landmarkPids)
{
	...
	std::string line, mot;
	std::string words[NB_LINES][NB_WORDS]; 
	int i,j, NbPoints;
